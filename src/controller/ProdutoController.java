...
public class ProdutoController extends javax.swing.JFrame {
    ...
    public void incrementaQantidadeItem(Produto produto, int quantidade) {
        if (verificarEstoque(produto, quantidade)) {
            for (ItemPedido ip : pedido.getItensPedido()) {
            ...
            JOptionPane.showMessageDialog(null, "A quantidade não está no intervalo válido");
        }
    }
    ...
    private void buscarPorDescricao() {
        ...
        ProdutoService produtoService = new ProdutoService();
        String descicao = JOptionPane.showInputDialog("Pesquisar por:");
        produtos = produtoService.buscarProdutosPorDescricao(descicao);
        if (!produtos.isEmpty()) {
            limparTabela();
    ...
    private void jbComprarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbComprarActionPerformed
        ...
        if (quantidadeNoPedido != 0) {
            incrementaQantidadeItem(produto, quantidade + quantidadeNoPedido);
        } else {
    ...
    private void jmFinalizarPedidoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jmFinalizarPedidoActionPerformed
        if (pedido.getItensPedido().size() >= 0) {
            ...
            JOptionPane.showMessageDialog(null, "Não foram adicionados itens ao carrinho");
        }
